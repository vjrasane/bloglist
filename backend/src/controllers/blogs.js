...
module.exports = sockets => {
  ...
  blogRouter.post('/', async (req, res) => {
    ...
    try {
      ...
      if (!user) return res.status(400).json({ error: 'inexistent user' })
  ...
  blogRouter.post('/:id/comments', async (req, res) => {
    ...
    try {
      ...
      if (!body.text) return res.status(400).json({ error: 'missing fields' })

      const blog = await Blog.findById(req.params.id)

      if (!blog) return res.status(400).json({ error: 'nonexistent blog' })

      const comment = new Comment({ ...body, user: req.token.id })
  ...
  blogRouter.delete('/:id', async (req, res) => {
    try {
      ...
      const blog = await Blog.findById(req.params.id)
      if (!blog) return res.status(400).json({ error: 'inexistent blog' })

      if (blog.user && blog.user.toString() !== req.token.id) {
        return res
          .status(401)
          .json({ error: "cannot remove another user's blog" })
      }
